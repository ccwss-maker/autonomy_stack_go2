...
M3D get_current_rotation_matrix()
{
    if (!use_imu_as_input)
    {
        return kf_output.x_.rot.normalized().toRotationMatrix();
    }
    return kf_input.x_.rot.normalized().toRotationMatrix();
}

V3D get_current_world_velocity()
{
    if (!use_imu_as_input)
    {
...
V3D get_current_body_angular_velocity()
{
    if (!use_imu_as_input)
    {
...
{
    const M3D rot = get_current_rotation_matrix();
    const V3D vel_world = get_current_world_velocity();
    const V3D vel_body = rot.transpose() * vel_world;
    const V3D omg_body = get_current_body_angular_velocity();

    out.linear.x = vel_body(0);
    out.linear.y = vel_body(1);
    out.linear.z = vel_body(2);
    out.angular.x = omg_body(0);
    out.angular.y = omg_body(1);
    out.angular.z = omg_body(2);
}

void publish_odometry(
    const rclcpp::Publisher<nav_msgs::msg::Odometry>::SharedPtr pubOdomAftMapped,
    const rclcpp::Publisher<unitree_go::msg::SportModeState>::SharedPtr pubSportModeState)
{
    ...
    if (velocity_pub_en)
    {
        ...
        sport_mode_state.stamp.sec = odomAftMapped.header.stamp.sec;
        sport_mode_state.stamp.nanosec = odomAftMapped.header.stamp.nanosec;
        sport_mode_state.velocity[0] = static_cast<float>(odomAftMapped.twist.twist.linear.x);
        sport_mode_state.velocity[1] = static_cast<float>(odomAftMapped.twist.twist.linear.y);
        sport_mode_state.velocity[2] = static_cast<float>(odomAftMapped.twist.twist.linear.z);
        pubSportModeState->publish(sport_mode_state);
    }
        ...
        if (!use_imu_as_input)
        {
            ...
            for (k = 0; k < time_seq.size(); k++)
            {
                ...
                if (publish_odometry_without_downsample)
                {
                    ...
                    publish_odometry(pubOdomAftMapped, pubSportModeState);
                    if (runtime_pos_log)
                    {
